...
Before we willy-nilly run a random container, let's take a look at the image will be using: https://hub.docker.com/r/docker/getting-started
...
1. Go here to any GitHub repo. It really doesn't matter which, we just want the Code tab. For example, go here: https://github.com/RVerse-Tutorials/Test
...
* [Docker Desktop](https://www.docker.com/products/docker-desktop/) If you are on a managed computer (e.g. government), you might not be able to install Docker due to a security risk associated with Docker running containers as root user.
...
* [Podman](https://podman.io/) is a more secure option since it does not run containers as root by default. Your organization is more likely to allow Podman. On my Mac M1, I have had a lot headaches running the rocker images though I did get it to run in the end.
